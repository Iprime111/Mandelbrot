...
union avxf_t {
    ...
    avxf_t (__m512 value);
    avxf_t (const float  value);
    avxf_t (const float  values [OPTIMIZATION_RATE]);
...
union avxu_t {
    ...
    avxu_t (const uint32_t  value);
    avxu_t (const uint32_t  values [OPTIMIZATION_RATE]);
...
inline avxf_t::avxf_t (const float  value):                     data (_mm512_set1_ps (value)) {}
inline avxf_t::avxf_t (const float  values [OPTIMIZATION_RATE]):data (_mm512_set_ps  (values [0],  values [1],  values [2],  values [3],
...
inline avxu_t::avxu_t (const uint32_t value):                     data (_mm512_set1_epi32 (value)) {}
inline avxu_t::avxu_t (const uint32_t values [OPTIMIZATION_RATE]):data (_mm512_set_epi32  (values [0],  values [1],  values [2],  values [3],
                                                                                           values [4],  values [5],  values [6],  values [7],
                                                                                           values [8],  values [9],  values [10], values [11],
                                                                                           values [12], values [13], values [14], values [15])) {}
